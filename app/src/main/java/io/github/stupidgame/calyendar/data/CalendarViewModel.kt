                ...
                viewModelScope.launch {
                    ...
                    combine(
                        ...
                        val totalMonthBalance =
                                FinancialCalculator.calculateDailyBalance(transactionsBefore) +
                                        FinancialCalculator.calculateDailyBalance(monthTransactions)
                        // 表示月より前の（過去の）すべての目標の合計
                        val priorGoals =
                                allGoals.filter { goal ->
                                    val goalDate = LocalDate.of(goal.year, goal.month + 1, goal.day)
                                    goalDate.isBefore(LocalDate.of(year, month + 1, 1))
                                }
                        val totalPriorGoalCost = priorGoals.sumOf { it.amount }
                        // 今月の目標カードに表示する「現在残高」（表示月までの全残高から過去のゴール分を引いた額）
                        val displayedCurrentBalance = totalMonthBalance - totalPriorGoalCost

                        CalendarUiState(year, month, dayStates, displayedCurrentBalance)
                    }
