...
class FinancialCalculatorTest {
    ...
    @Test
    fun `goal projection ignores goals before the window start date`() {
        val result =
        ...
        assertEquals(160_000L, result.currentBalance)
        ...
        assertEquals(110_000L, result.predictionDiff)
    }

    @Test
    fun `goal projection subtracts reached goals from the window before comparing later goals`() {
        val result =
            FinancialCalculator.calculateGoalProjection(
                rawBalance = 5_000L,
                allGoals =
